...
5. **Changelogs, every version bump:** Whenever `app/build.gradle.kts`'s `versionCode`/`versionName` is bumped, add a changelog entry for that release at `fastlane/metadata/android/<locale>/changelogs/<versionCode>.txt` — filename is the new `versionCode` — in each of `en-US`, `de-DE`, and `fr-FR`, kept content-identical across languages (translated, not copy-pasted). This is the one and only changelog location: `fastlane/metadata/android/` is the single source of truth for both Google Play (via `fastlane supply`) and F-Droid (which reads this same directory straight from the repo) — there is no second copy to keep in sync.

   Write it from the user's perspective (what changed, not which files), one bullet per change, "No user-facing changes" for a pure maintenance release, and keep every locale's entry at or under 500 characters (Google Play and F-Droid both cap release notes there). Do this as part of the same commit that bumps the version and creates the release tag — F-Droid builds from the exact tagged commit, so a changelog added after tagging won't be picked up until the next release. A version bump with no changelog entry is an incomplete release, the same way a feature without a `FEATURES.md` entry is.
