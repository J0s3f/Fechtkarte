...
([`config/allowed-licenses.json`](../config/allowed-licenses.json), task
[T0.3](NEXT_STEPS.md)) and fails the build if any dependency's licence isn't on the
allowlist below. The two should never disagree; if they do, this file is describing an
intent the build isn't actually enforcing, and that's a bug in the gate, not a footnote here.
...
D9 (see [`DESIGN_CHOICES.md`](../DESIGN_CHOICES.md)) settles this. Apache-2.0 is
F-Droid-acceptable, permits commercial and closed-source downstream reuse (unlike GPL-3.0),
and includes an explicit patent grant that plain MIT lacks — a reasonable default for a
project with no other licensing constraint pulling it toward MIT or GPL. Still to do: set the
licence in the F-Droid metadata when that submission is prepared (task T7.7), and remember
that Apache-2.0 requires the `NOTICE` file's contents to travel with any redistribution.
...
Removed from the scaffold: **Gson** (Apache-2.0, would have been fine — dropped for technical
reasons, see `DESIGN_CHOICES.md`).
...
thrusts. It is digitised from card artwork depicting 16th-century manuscript diagrams by
Joachim Meyer (MS A.4º.2, Lund; MS Var.82, Rostock) — see the dataset's own `source` field.
Facts about a historical pattern — "action 3 starts north-east, outer ring, and is a thrust" —
are not authored expression, and the diagrams themselves are centuries out of copyright.

It is numbers: no pixels, colours or glyphs from any other source survive into the dataset.
...
should replace it if this document or `docs/PLAN.md` §3.3 is updated again.
...
or fall back to Fechtkarte's own vector redraw of the diagram (the fallback docs/PLAN.md
already anticipated, which also suits the app's own visual identity better and sidesteps any
residual "faithful reproduction" nuance entirely). Downloading and bundling an actual scan
file is a distinct decision from clearing its licence — not done as part of this research pass.
