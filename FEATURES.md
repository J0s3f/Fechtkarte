...
**A library of all 109 historical drill cards.** Opens straight to the Library screen: a
Drills tab (the 44 historical drills, each viewable for either hand, filterable by action
count and thrust count) and a Techniques tab (the 21 technique cards, filterable by
technique). Each tab has first/previous/next/last, ±10, and a random pick — the same controls
the original 1990s app offered, now over 44/21 distinct entries instead of 109 raw ones, since
half the original count was the same drill mirrored for the other hand.
...
across the 109 historical cards. All of it persists across restarts.
...
**Screen reader support.** A generated or historical card has no text of its own — it's pure
...
Tracked in [`roadmap.md`](roadmap.md), specified in [`docs/PLAN.md`](docs/PLAN.md), and broken
into tasks in [`docs/NEXT_STEPS.md`](docs/NEXT_STEPS.md).

- A "Home" screen with its own title treatment, beyond the launcher label (docs/PLAN.md §7 —
  T5.3 deferred building any Home screen at all; Library is still the start destination)
- A Sources screen crediting the manuscripts (blocked on licence clearance)
- F-Droid submission — Fastlane text metadata, the notices screen, and the icon/feature-graphic
  image assets are done; phone screenshots (needs a real device/emulator capture, not test
  fixtures) and real release signing are still open
